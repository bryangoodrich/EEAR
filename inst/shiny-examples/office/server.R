...
shinyServer (function(input, output) {
    ...
    # Raw Data Input
    original <- reactive({EEAR::OfficeMeter})
    ...
    output$file_loaded <- reactive({TRUE})
...
#     })
    ...
    output$dataSummary <- renderPrint ({ summary(prepared()) })
    
    output$buildingMetrics <- renderDataTable ({
        metrics(prepared(), group = tolower(input$facets))
    })
